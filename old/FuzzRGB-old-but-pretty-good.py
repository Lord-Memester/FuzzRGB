...
print(f"Detected thread count: {multiprocessing.cpu_count()}\n") #We get the number of threads to most make use of the multithreading, spreading the workload across all the CPU's threads as optimally as I can manage at this time.

threads = multiprocessing.cpu_count()
i = 0
subdivisions = 1 # initializing the value so the IDE doesn't yell at me
...
img = Image.new('RGB', [width, height],(127,127,127)) #Image.new('RGBA', [width, height],(0,0,0, 0))
...
def worker_x(): # used for landscape images
    ...
    while ( xcoord <= active_subdivision_end_x ):
        ...
        #alpha = 127 #random.randrange(0,256,1) #alpha value
...
Other than that, using the GPU would be an improvement."""
